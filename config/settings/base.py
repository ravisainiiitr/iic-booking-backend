...
# API-first URLConf no longer mounts django-allauth account routes (account_login).
# Session-protected views (admin, legacy user pages) redirect to Django admin login.
...
# Fail fast if Redis is unreachable so publishers don't hang the web process.
...
RESEARCH_COPILOT_EMBEDDING_TIMEOUT_SECONDS = env.int("RESEARCH_COPILOT_EMBEDDING_TIMEOUT_SECONDS", default=120)
RESEARCH_COPILOT_VECTOR_STORE = env("RESEARCH_COPILOT_VECTOR_STORE", default="django_orm")
RESEARCH_COPILOT_VECTOR_SCAN_LIMIT = env.int(
    "RESEARCH_COPILOT_VECTOR_SCAN_LIMIT", default=env.int("RESEARCH_COPILOT_SCAN_LIMIT", default=5000)
)
# Anonymous (signed-out) Copilot. Independent of the signed-in pilot allowlist.
RESEARCH_COPILOT_PUBLIC_ENABLED = env.bool("RESEARCH_COPILOT_PUBLIC_ENABLED", default=False)
# Equipment manual PDFs (admin upload → text extraction → equipment-scoped retrieval).
RESEARCH_COPILOT_MANUAL_MAX_BYTES = env.int("RESEARCH_COPILOT_MANUAL_MAX_BYTES", default=50 * 1024**2)
RESEARCH_COPILOT_MANUAL_MAX_PAGES = env.int("RESEARCH_COPILOT_MANUAL_MAX_PAGES", default=1500)
# Under the private research/ prefix the application credentials are already permitted to write.
RESEARCH_COPILOT_MANUAL_PREFIX = env("RESEARCH_COPILOT_MANUAL_PREFIX", default="research/copilot-manuals")
RESEARCH_COPILOT_MANUAL_URL_EXPIRY_SECONDS = env.int("RESEARCH_COPILOT_MANUAL_URL_EXPIRY_SECONDS", default=300)
