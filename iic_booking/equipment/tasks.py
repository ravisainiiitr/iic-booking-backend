...
def auto_complete_bookings_with_data_after_end() -> int:
    """
    Equipment-gated auto-complete after slot end when material result data exists.

    Delegates to booking_completion_service (row lock + existing completion email).
    Does not scan historical completed bookings. Skips active Remote Analysis sessions.
    """
    from .booking_completion_service import (
        auto_complete_candidate_queryset,
        try_auto_complete_booking,
    )
    ...
    for booking in auto_complete_candidate_queryset():
        scanned += 1
        try:
            ok, reason = try_auto_complete_booking(booking)
        except Exception:
            logger.exception(
        ...
        if ok:
            ...
            skipped += 1
            logger.info(
                "auto_complete_bookings_with_data_after_end: skip booking_id=%s reason=%s",
                booking.booking_id,
                reason or "skipped",
            )

    logger.info(
        "auto_complete_bookings_with_data_after_end: scanned=%d completed=%d skipped=%d",
        scanned,
        completed,
        skipped,
    )
