...
@pytest.mark.django_db
@override_settings(AWS_STORAGE_BUCKET_NAME="")
def test_auto_complete_skips_when_end_time_not_reached(tmp_path, settings):
    settings.DSA_UPLOAD_STORAGE_ROOT = str(tmp_path)
    owner = UserFactory()
    equipment = _equipment(auto_complete_booking=True)
    booking = _booking(owner, equipment)
    ...
    slot_master = SlotMaster.objects.create(
        equipment=equipment,
        slot_number=1,
        open_time=now.time().replace(microsecond=0),
        close_time=(now + timedelta(hours=2)).time().replace(microsecond=0),
        is_active=True,
    )
    DailySlot.objects.create(
        slot_master=slot_master,
...
@pytest.mark.django_db
@override_settings(AWS_STORAGE_BUCKET_NAME="")
def test_auto_complete_skips_missing_workspace_and_hidden_only(tmp_path, settings):
    settings.DSA_UPLOAD_STORAGE_ROOT = str(tmp_path)
    owner = UserFactory()
    equipment = _equipment(auto_complete_booking=True)
    booking = _booking(owner, equipment)
    ...
    assert auto_complete_bookings_with_data_after_end() == 0
    booking.refresh_from_db()
    assert booking.status == BookingStatus.BOOKED

    _seed_workspace(booking, agent)
    ResultAttachment.objects.filter(result__booking_id=booking.pk).delete()
    _seed_dsa_attachment(
        booking=booking, agent=agent, storage_root=tmp_path, filename=".hidden", content=b"x"
    )
    assert auto_complete_bookings_with_data_after_end() == 0
    booking.refresh_from_db()
    assert booking.status == BookingStatus.BOOKED
...
@pytest.mark.django_db
@override_settings(AWS_STORAGE_BUCKET_NAME="")
def test_auto_complete_skips_empty_result_directory(tmp_path, settings):
    settings.DSA_UPLOAD_STORAGE_ROOT = str(tmp_path)
    owner = UserFactory()
    equipment = _equipment(auto_complete_booking=True)
    booking = _booking(owner, equipment)
...
@pytest.mark.django_db
@override_settings(AWS_STORAGE_BUCKET_NAME="")
def test_auto_complete_twice_is_idempotent(tmp_path, settings, monkeypatch):
    settings.DSA_UPLOAD_STORAGE_ROOT = str(tmp_path)
    owner = UserFactory()
    equipment = _equipment(auto_complete_booking=True)
    booking = _booking(owner, equipment)
